...
import {
  getCurrentWord,
  getSuggestions as getDefaultSuggestions,
...
const SuggestionIcon = styled("span", {
  ...
  fontWeight: "bold",
  backgroundColor:
    type === "P" ? "rgba(0, 122, 204, 0.2)" : "rgba(184, 115, 51, 0.2)",
...
export default function Suggestions() {
  ...
  const { caretCoordinates, code, position, suggestions, suggestionIndex } =
    state;
  ...
  useEffect(() => {
    const isInsideToken = state.tokens.some(
      (t) =>
        position !== null && position >= t.range[0] && position < t.range[1],
    );
    ...
    if (
      ...
      const isContiguous =
        ...
        Math.abs(position - lastPositionRef.current) <= 1;
