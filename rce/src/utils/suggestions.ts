export const CSS_PROPERTIES = [
    ...
    "content", "counter-increment", "counter-reset", "counter-set", "cursor",
    ...
    "font-style", "font-variant", "font-variant-alternates", "font-variant-caps",
    "font-variant-east-asian", "font-variant-ligatures", "font-variant-numeric",
    "font-variant-position", "font-variation-settings", "font-weight", "forced-color-adjust",
    ...
    "image-rendering", "image-resolution", "initial-letter", "inline-size", "inset",
    ...
    "mask-size", "mask-type", "max-block-size", "max-height", "max-inline-size",
    ...
    "pointer-events", "position", "print-color-adjust", "quotes", "resize", "right",
    "rotate", "row-gap", "ruby-align", "ruby-position", "scale", "scroll-behavior",
    ...
    "text-shadow", "text-size-adjust", "text-transform", "text-underline-offset",
...
export function getCurrentWord(code: string, position: number): { word: string; start: number; end: number } {
    let start = position;
    // Allow hyphen and alphanumeric for CSS properties
...
export function getSuggestions(word: string): string[] {
    ...
    // Exact matches or startsWith first, then contains
    const primary = CSS_PROPERTIES.filter(prop => prop.startsWith(lowerWord));
    const secondary = CSS_PROPERTIES.filter(prop => !prop.startsWith(lowerWord) && prop.includes(lowerWord));
    
    return [...primary, ...secondary].slice(0, 15);
}
